...
object JSONReader {
  ...
  def getAllRoads(environment : urban_elements) : List[road] = {
    environment match {
    case (urban_elements(_, roads,_,_,_,_,_,_)) =>
    return roads
    }
  }
  ...
  def getAllLanes(environment : urban_elements) : List[lane] = {
    environment match {
    case (urban_elements(_, _,lanes,_,_,_,_,_)) =>
    return lanes
    }
  }
  ...
  def getAllCrossroads(environment : urban_elements) : List[crossroad] = {
      environment match {
      case (urban_elements(_, _,_,crossroads,_,_,_,_)) =>
      return crossroads
      }
  }
  ...
  def getAllPedestrianCrossroads(environment : urban_elements) : List[pedestrian_crossroad] = {
      environment match {
      case (urban_elements(_, _,_,_,pedestrian_crossroads,_,_,_)) =>
  ...
  def getAllBusStops(environment : urban_elements) : List[bus_stop] = {
      environment match {
      case (urban_elements(_, _,_,_,_,bus_stops,_,_)) =>
      return bus_stops
      }
  }
  ...
  def getAllTramStops(environment : urban_elements) : List[tram_stop] = {
      environment match {
      case (urban_elements(_, _,_,_,_,_,tram_stops,_)) =>
      return tram_stops
      }
  }
  ...
  def getAllZones(environment : urban_elements) : List[zone] = {
      environment match {
      case (urban_elements(_, _,_,_,_,_,_,zones)) =>
      return zones
      }
  }
  
  // metodi get per dimensions
  
  def getDimensionsX(environment : urban_elements) : Int = {
    return environment.dimensions.x
  }
  
  def getDimensionsY(environment : urban_elements) : Int = {
    return environment.dimensions.y
  }
  ...
  def getLaneBusRoutes(environment : urban_elements, id : String) : List[Int] = {
    environment match {
    case (urban_elements(_, _,lanes,_,_,_,_,_)) =>
    val lane = lanes.find { lane => lane.id == id }
    if(lane.equals(None)) {
  ...
  def getTramStop(environment : urban_elements, id : String) : Option[tram_stop] = {
    environment match {
    case (urban_elements(_, _,_,_,_,_,tram_stops,_)) =>
